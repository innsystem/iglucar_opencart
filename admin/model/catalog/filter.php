...
class ModelCatalogFilter extends Model {
		...
		if (isset($data['filter'])) {
			foreach ($data['filter'] as $filter) {
				...
				foreach ($filter['filter_description'] as $language_id => $filter_description) {
					...
					if (isset($filter_description['images']) && is_array($filter_description['images'])) {
						foreach ($filter_description['images'] as $image) {
							if (!empty($image['image'])) {
								$this->db->query("INSERT INTO " . DB_PREFIX . "filter_description_images SET filter_description_id = '" . (int)$filter_description_id . "', filter_id = '" . (int)$filter_id . "', language_id = '" . (int)$language_id . "', image = '" . $this->db->escape($image['image']) . "', sort_order = '" . (int)$image['sort_order'] . "'");
							}
						}
					}
				}
	...
	public function editFilter($filter_group_id, $data) {
		...
		// Deletar filtros e suas descrições
		...
		// Deletar todas as imagens relacionadas a este grupo de filtros
		$this->db->query("DELETE fdi FROM " . DB_PREFIX . "filter_description_images fdi 
						 INNER JOIN " . DB_PREFIX . "filter f ON fdi.filter_id = f.filter_id 
			...
			foreach ($data['filter'] as $filter) {
				...
				foreach ($filter['filter_description'] as $language_id => $filter_description) {
					...
					if (isset($filter_description['images']) && is_array($filter_description['images'])) {
						foreach ($filter_description['images'] as $image) {
							if (!empty($image['image'])) {
								// Verificar se esta imagem já existe para este filter_description_id
								$existing_image = $this->db->query("SELECT COUNT(*) as total FROM " . DB_PREFIX . "filter_description_images WHERE filter_description_id = '" . (int)$filter_id . "' AND image = '" . $this->db->escape($image['image']) . "'");
								
								if ($existing_image->row['total'] == 0) {
									// Inserir apenas se não existir
									$this->db->query("INSERT INTO " . DB_PREFIX . "filter_description_images SET filter_description_id = '" . (int)$filter_id . "', filter_id = '" . (int)$filter_id . "', language_id = '" . (int)$language_id . "', image = '" . $this->db->escape($image['image']) . "', sort_order = '" . (int)$image['sort_order'] . "'");
								}
							}
						}
					}
	...
	public function deleteFilter($filter_group_id) {
		...
		$filter_ids = $this->db->query("SELECT filter_id FROM " . DB_PREFIX . "filter WHERE filter_group_id = '" . (int)$filter_group_id . "'");
		if ($filter_ids->num_rows > 0) {
			foreach ($filter_ids->rows as $filter) {
				$this->db->query("DELETE FROM " . DB_PREFIX . "filter_description_images WHERE filter_id = '" . (int)$filter['filter_id'] . "'");
			}
		}
	...
	public function getFilterDescriptions($filter_group_id) {
		...
		foreach ($filter_query->rows as $filter) {
			...
			foreach ($filter_description_query->rows as $filter_description) {
				// Buscar imagens para esta descrição
				$images_query = $this->db->query("SELECT * FROM " . DB_PREFIX . "filter_description_images WHERE filter_id = '" . (int)$filter['filter_id'] . "' AND language_id = '" . (int)$filter_description['language_id'] . "' ORDER BY sort_order ASC");
