...
class ControllerSaleOrder extends Controller {
	...
	protected function getList() {
		...
		foreach ($results as $result) {
			$data['orders'][] = array(
				...
				'ver'      => $this->checkF($result['order_id']),'nfe'      => $this->checkN($result['order_id']),'post'      => $this->checkP($result['order_id']),
				'order_status'  => $result['order_status'] ? $result['order_status'] : $this->language->get('text_missing'),
				'total'         => $this->currency->format($result['total'], $result['currency_code'], $result['currency_value']),
				'date_added'    => date($this->language->get('date_format_short'), strtotime($result['date_added'])),
				'date_modified' => date($this->language->get('date_format_short'), strtotime($result['date_modified'])),
				'frete'          => $this->url->link('sale/order/frete', 'user_token=' . $this->session->data['user_token'] . '&order_id=' . $result['order_id'] . $url, true),'frete2'          => $this->url->link('sale/order/frete2', 'user_token=' . $this->session->data['user_token'] . '&order_id=' . $result['order_id'] . $url, true),
		...
		if ($this->config->get('shipping_melhorenvios_tde') == 0 ){
	...
	//   die();
	  ...
	  if (array_key_exists("errors", $retornou) || array_key_exists("error", $retornou)) {
		$this->error['warning'] = $retornou['error'];
