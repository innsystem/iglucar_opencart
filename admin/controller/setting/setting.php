...
//			}
		...
		if (isset($this->error['sales_count'])) {
			$data['error_sales_count'] = $this->error['sales_count'];
		} else {
			$data['error_sales_count'] = '';
		}
		...
		if (isset($this->request->post['config_sales_count'])) {
			$data['config_sales_count'] = $this->request->post['config_sales_count'];
		} else {
			$data['config_sales_count'] = $this->config->get('config_sales_count');
		}
		...
		if (isset($this->request->post['config_manual_pdf'])) {
			$data['config_manual_pdf'] = $this->request->post['config_manual_pdf'];
		} else {
			$data['config_manual_pdf'] = $this->config->get('config_manual_pdf');
		}

		// URL para download do PDF atual
		if ($data['config_manual_pdf']) {
			$data['manual_pdf_url'] = HTTP_CATALOG . 'image/catalog/docs/' . $data['config_manual_pdf'];
		} else {
			$data['manual_pdf_url'] = '';
		}
	...
	protected function validate() {
		...
		if (!empty($this->request->post['config_telephone2']) && ((utf8_strlen($this->request->post['config_telephone2']) < 3) || (utf8_strlen($this->request->post['config_telephone2']) > 32))) {
			$this->error['telephone2'] = $this->language->get('error_telephone');
		}

		if (!empty($this->request->post['config_telephone3']) && ((utf8_strlen($this->request->post['config_telephone3']) < 3) || (utf8_strlen($this->request->post['config_telephone3']) > 32))) {
			$this->error['telephone3'] = $this->language->get('error_telephone');
		}

		if (!empty($this->request->post['config_sales_count']) && (!is_numeric($this->request->post['config_sales_count']) || $this->request->post['config_sales_count'] < 0)) {
			$this->error['sales_count'] = 'Número de vendas deve ser um valor numérico positivo';
		}
	...
	public function uploadPdf() {
		...
		if (!ini_get('file_uploads')) {
			$json['error'] = 'Upload de arquivos está desabilitado no servidor!';
		}
		...
		if ($upload_max_bytes < 52428800) { // 50MB
			$json['error'] = 'Limite de upload do servidor muito baixo. Atual: ' . $upload_max_filesize . ', Necessário: 50MB';
		}
		
		if ($post_max_bytes < 52428800) { // 50MB
			$json['error'] = 'Limite POST do servidor muito baixo. Atual: ' . $post_max_size . ', Necessário: 50MB';
		}

		// Directory for PDF files
		$directory = DIR_IMAGE . 'catalog/docs/';
		...
		if (!$json) {
			...
			error_log('Upload Debug - Files: ' . print_r($this->request->files, true));
			error_log('Upload Debug - POST: ' . print_r($this->request->post, true));
			...
			if (!empty($this->request->files['file']['name']) && is_file($this->request->files['file']['tmp_name'])) {
				...
				// Allowed file extension types (only PDF)
				$allowed = array('pdf');
				...
				// If file exists, try to remove it first (allow replacement)
				...
				// Check file size
				if ($this->request->files['file']['size'] > 50485760) { // 50MB limit
					$json['error'] = 'O arquivo é muito grande. Tamanho máximo: 50MB';
				}
			} else {
				// More detailed error checking
				if (empty($this->request->files['file']['name'])) {
					$json['error'] = 'Nenhum arquivo foi selecionado!';
				} elseif (!is_file($this->request->files['file']['tmp_name'])) {
					$json['error'] = 'Arquivo temporário não encontrado. Verifique as configurações de upload do servidor.';
					...
					$json['error'] = 'Erro no upload: ' . (isset($upload_errors[$this->request->files['file']['error']]) ? $upload_errors[$this->request->files['file']['error']] : 'Erro desconhecido');
				} else {
		...
		if (!$json) {
			...
			if (move_uploaded_file($this->request->files['file']['tmp_name'], $directory . $filename)) {
				$json['success'] = 'Arquivo enviado com sucesso!';
				$json['filename'] = $filename;
				$json['url'] = HTTP_CATALOG . 'image/catalog/docs/' . $filename;
			} else {
	...
	public function clearPdf() {
		...
		if (isset($this->request->post['filename']) && !empty($this->request->post['filename'])) {
			$filename = $this->request->post['filename'];
			$filepath = DIR_IMAGE . 'catalog/docs/' . $filename;
			
			// Check if file exists and remove it
			if (file_exists($filepath)) {
				if (@unlink($filepath)) {
					$json['success'] = 'Arquivo removido com sucesso!';
				} else {
					$json['error'] = 'Erro ao remover o arquivo!';
				}
			} else {
				$json['success'] = 'Arquivo não encontrado.';
			}
		} else {
