...
class ControllerCatalogCategory extends Controller
{
	...
	public function add()
	{
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validateForm()) {
			if ($this->request->post['parent_id'] > 0) {
				$this->request->post['category_seo_url'][(int)$this->config->get('config_store_id')][$this->config->get('config_language_id')] = $this->slugify($this->request->post['path'] . '-' . $this->request->post['category_description'][$this->config->get('config_language_id')]['name']);
			} else {
				$this->request->post['category_seo_url'][(int)$this->config->get('config_store_id')][$this->config->get('config_language_id')] = $this->slugify($this->request->post['category_description'][$this->config->get('config_language_id')]['name']);
			}
	...
	public function autocomplete()
	{
		...
		if (isset($this->request->get['filter_name'])) {
			...
			$filter_data = array(
				...
				'order'       => 'ASC',
				'start'       => 0,
				'limit'       => $this->config->get('config_limit_autocomplete') ?? 5,
			);
	...
	public function import()
	{
		...
		if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_FILES['file'])) {
			...
			if ($this->validateUpload($file)) {
				...
				$json['success'] = 'Categorias importadas com sucesso!';
			} else {
				$json['error'] = 'Erro ao importar categorias. Verifique o arquivo.';
			}
		} else {
	...
	{
		$allowed_mime = ['application/vnd.ms-excel', 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet'];
		$file_mime = mime_content_type($file);
		return in_array($file_mime, $allowed_mime);
	}

	private function sanitizeSeoUrl($string) {
		...
		$unwanted_array = array(
			...
			'Ì'=>'I', 'Í'=>'I', 'Î'=>'I', 'Ï'=>'I', 'Ñ'=>'N', 'Ò'=>'O', 'Ó'=>'O', 'Ô'=>'O', 'Õ'=>'O', 'Ö'=>'O', 'Ø'=>'O', 'Ù'=>'U', 'Ú'=>'U', 'Û'=>'U', 'Ü'=>'U', 'Ý'=>'Y', 'Þ'=>'B',
			'à'=>'a', 'á'=>'a', 'â'=>'a', 'ã'=>'a', 'ä'=>'a', 'å'=>'a', 'æ'=>'a', 'ç'=>'c', 'è'=>'e', 'é'=>'e', 'ê'=>'e', 'ë'=>'e',
			'ì'=>'i', 'í'=>'i', 'î'=>'i', 'ï'=>'i', 'ð'=>'o', 'ñ'=>'n', 'ò'=>'o', 'ó'=>'o', 'ô'=>'o', 'õ'=>'o', 'ö'=>'o', 'ø'=>'o', 'ù'=>'u', 'ú'=>'u', 'û'=>'u', 'ü'=>'u', 'ý'=>'y', 'þ'=>'b', 'ÿ'=>'y'
		);
		...
		// Remover múltiplos hífens seguidos
	...
	private function processImport($file)
	{
		...
		foreach ($rows as $index => $row) {
			...
			$category_name = isset($row['A']) ? trim($row['A']) : '';
			$parent_name = isset($row['B']) ? trim($row['B']) : '';

			if (!empty($category_name)) {
				$parent_id = 0;
				if (!empty($parent_name)) {
					$parent_id = $this->model_catalog_category->getCategoryIdByName($parent_name);
				}
				...
				$this->model_catalog_category->addCategory([
					...
					'sort_order' => 0,
					'status' => 1,
					'category_store' => [0], // Loja padrão
					'category_seo_url' => [
						0 => [ // store_id 0 (loja padrão)
							2 => $this->sanitizeSeoUrl($category_name) // Gera um slug com base no nome da categoria
						]
					],
					'category_layout' => [
	...
	public function toggleStatus() {
		...
		if ($this->request->server['REQUEST_METHOD'] == 'POST' && isset($this->request->post['category_id']) && isset($this->request->post['status'])) {
			$this->load->model('catalog/category');
			
			$category_id = (int)$this->request->post['category_id'];
			$new_status = (int)$this->request->post['status'];
	
			// Atualiza o status na Model
			$this->model_catalog_category->updateCategoryStatus($category_id, $new_status);
