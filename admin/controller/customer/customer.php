...
class ControllerCustomerCustomer extends Controller {
	...
	protected function getList() {
		...
		foreach ($results as $result) {
			...
			// Buscar documento (custom_field 1 ou 2)
			$document = '';
			$custom_field = json_decode($result['custom_field'], true);
			if (is_array($custom_field)) {
				if (isset($custom_field[1]) && !empty($custom_field[1])) {
					$document = $custom_field[1];
				} elseif (isset($custom_field[2]) && !empty($custom_field[2])) {
					$document = $custom_field[2];
				}
			}
