...
class ControllerProductProduct extends Controller
{
	...
	public function index()
	{
		...
		// Buscar informações de filtros se houver parâmetros search ou filter
		$data['filter_info'] = array();
		$data['filter_images'] = array();
		
		if (isset($this->request->get['search']) || isset($this->request->get['filter'])) {
			// Debug: verificar se os parâmetros estão sendo recebidos
			$data['debug_search'] = isset($this->request->get['search']) ? $this->request->get['search'] : 'não definido';
			$data['debug_filter'] = isset($this->request->get['filter']) ? $this->request->get['filter'] : 'não definido';
			...
			if (isset($this->request->get['search'])) {
				...
				$search_term = $this->request->get['filter'];
			}
			
			// Converter %20 para espaços e decodificar entidades HTML
			$search_term = html_entity_decode(urldecode($search_term), ENT_QUOTES, 'UTF-8');
			$data['debug_search_decoded'] = $search_term;
			
			// Buscar filtro com nome exato
			$filter_info = $this->model_catalog_filter->getFilterByName($search_term);
			
			if ($filter_info) {
				$data['filter_info'] = $filter_info;
				
				// Buscar imagens relacionadas ao filtro
				$filter_images = $this->model_catalog_filter->getFilterImages($filter_info['filter_id']);
				$data['filter_images'] = $filter_images;
			} else {
				$data['debug_filter_not_found'] = 'Filtro não encontrado para: ' . $search_term;
			}
		}
		...
		if ($product_info) {
			...
			if ($this->config->get('config_product_weight')) {
				$data['product_weight_show'] = true;
			} else {
				$data['product_weight_show'] = false;
			}

			$length_unit = $this->model_catalog_product->getLenghtProduct($product_info['length_class_id']);

			$data['product_width'] = number_format($product_info['width'], 2);
			$data['product_height'] = number_format($product_info['height'], 2);
			$data['product_length'] = number_format($product_info['length'], 2);
			$data['product_length_unit'] = $length_unit;
			...
			if ($product_info['image']) {
				$data['small'] = $this->model_tool_image->resize($product_info['image'], 500, 500);
			} else {
				$data['small'] = '';
			}
			...
			foreach ($results as $result) {
				$data['images'][] = array(
					...
					'small' => $this->model_tool_image->resize($result['image'], 500, 500),
				...
				foreach ($option['product_option_value'] as $option_value) {
					...
					if ($option_value) {
						...
						$product_option_value_data[] = array(
							...
							'product_option_image_thumb'    => $this->model_tool_image->resize($option_value['product_option_image'], 50, 50),
							'product_option_image'    => $this->model_tool_image->resize($option_value['product_option_image'], 400, 400),
							'product_option_image_zoom'    => $this->model_tool_image->resize($option_value['product_option_image'], 500, 500),
							'price'                   => $price,
			...
			//product variated
			$results = $this->model_catalog_product->getProductVariated($this->request->get['product_id']);
			foreach ($results as $result) {
				if ($result['image']) {
			...
			// Adicionar número de vendas configurado
			$data['sales_count'] = $this->config->get('config_sales_count') ? $this->config->get('config_sales_count') : 1347;
