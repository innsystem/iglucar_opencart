<?php
class ModelExtensionShippingMelhorenvios extends Model {
	function getQuote($address) {
		$this->load->language('extension/shipping/melhorenvios');

		$query = $this->db->query("SELECT * FROM " . DB_PREFIX . "zone_to_geo_zone WHERE geo_zone_id = '" . (int)$this->config->get('shipping_melhorenvios_geo_zone_id') . "' AND country_id = '" . (int)$address['country_id'] . "' AND (zone_id = '" . (int)$address['zone_id'] . "' OR zone_id = '0')");

		if (!$this->config->get('shipping_melhorenvios_geo_zone_id')) {
			$status = true;
        ...
        $variavel = $this->config->get('shipping_melhorenvios_cargo');
		...
		if($this->config->get('shipping_melhorenvios_title') !='') {
			$titulo = $this->config->get('shipping_melhorenvios_title');
		} else {
		...
		$at = true;
		if ($this->config->get('shipping_melhorenvios_debug')) {	
		$this->log->write('PAYLOAD: MELHOR ENVIO API - '. json_encode($header) .' - '. $json_convert);
		}
		}
		
		$cargo = $this->config->get('shipping_melhorenvios_cargo');
        ...
        if (!array_key_exists("error", $value) && in_array($value['id'], $cargo)) {
			if ($this->config->get('shipping_melhorenvios_days') > 0) {
			$dias = $value['delivery_time'] + $this->config->get('shipping_melhorenvios_days');
			} else {
			...
			$valorad = str_replace(',', '.', $this->config->get('shipping_melhorenvios_adic'));
			if ($this->config->get('shipping_melhorenvios_adic') > 0 && $this->config->get('shipping_melhorenvios_tipo') == 1 ) {
			$st = $value['price'];
		    $st2 = ($valorad/100)* $st;
			$cost = $value['price'] + $st2;	
			} else if ($this->config->get('shipping_melhorenvios_adic') > 0 && $this->config->get('shipping_melhorenvios_tipo') == 0 ) {
		...
		$quote_data[$value['id']] = array(
				...
				'cost'         => $cost,
				'tax_class_id' => $this->config->get('shipping_melhorenvios_tax_class_id'),
				'text'         => $this->currency->format($this->tax->calculate($cost, $this->config->get('shipping_melhorenvios_tax_class_id'), $this->config->get('config_tax')), $this->session->data['currency']) . ' <b>entrega em até <span style="color:#019432;">'. $dias .'</span>'. $tdias.'</b>'
			);
		}
		}
		}
		...
		if ($quote_data) {
			$method_data = array(
				...
				'title'      => $titulo,
				'quote'      => $quote_data,
				'sort_order' => $this->config->get('shipping_melhorenvios_sort_order'),
				'error'      => false
			);
		}
